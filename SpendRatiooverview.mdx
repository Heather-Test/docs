...
* Add or change a Spend Ratio (*Link*)
...
Spend Ratios are one of the three Bid Model components that you can manage for your campaigns. Refer to Bid Modifier overview (*Link*) and Dimension Priority overview (*Link*) for more details on the other ways that you can apply Bid Model settings. 
...
The steps you’ll follow to add or edit a Bid Model setting will differ based on the dimension category (*Link*) for which you’d like to apply the Dimension Priority, Bid Modifier, or Spend Ratio. 
...
Note that you can only choose a strict Spend Ratio if no Dimension Priority (*Link*) is assigned to the same dimension(s).
...
\* *Refer to Others (link) for more information.*&#x20;
